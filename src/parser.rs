...
{
    let nil = primitive::just(Token::LParen)
        .then(primitive::just(Token::RParen))
        .map(|_| Value::Nil);
    ...
    recursive::recursive(|s_expression| {
        ...
        let list = s_expression
            ...
            .delimited_by(
                primitive::just(Token::LParen),
                primitive::just(Token::RParen),
            );

        let dot_notation = s_expression
            .clone()
            .then_ignore(primitive::just(Token::Dot))
            .then(s_expression)
            .delimited_by(
                primitive::just(Token::LParen),
                primitive::just(Token::RParen),
